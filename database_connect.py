...
class DatabaseManager:
    ...
    def data_add(self, file_name, add_or_update_time):
        ...
        try:
            return self.cursor.execute("INSERT INTO files (filename, datetime) "
                                "VALUES (?, ?)",
                                (file_name, add_or_update_time)
                                       )
        except IntegrityError:
            return self.cursor.execute("UPDATE files "
                                       "SET datetime = ? "
                                       "WHERE filename =?",
    ...
    def data_delete(self, file_name):
        ...
        return self.cursor.execute("DELETE "
                                   "FROM files "
                                   "WHERE filename = ?",
                                   (file_name,)
                                   )
...
def sql_req(func, **kwargs):
    ...
    with DatabaseManager("database.db") as db:
        if func in ("add", "update"):
            return db.data_add(file_name=data["file"], add_or_update_time=data["add_or_update_datetime"])
        elif func == "del":
